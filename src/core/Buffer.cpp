...
namespace reactor::base
{
    ...
    std::string::size_type Buffer::find(const std::string_view substr) const
    {
        ...
        return buffer.find(
            substr); // 由于读取默认都是从未消费开始即readIndex_开始，所有这里返回的都是相对值
    }
    ...
    {
        const auto readable = readableBytes();
        if (offset > readable || len > readable - offset)
            return "";
    ...
    void Buffer::makeSpace_(size_t len)
    {
        ...
        else
        {
            ...
            std::copy(begin_() + readIndex_, begin_() + writeIndex_, begin_() + kCheapPrepend);
            readIndex_ = kCheapPrepend;
